...
class PEFTPolicy(SchedulingAlgorithm):
    def __init__(self):
    ...
    def select_task_and_host(
        ...
        best_oeft: Optional[float] = None
        best_eft: Optional[float] = None
        ...
        for task in ready_tasks:
            ...
            cp_rem = float(getattr(task, "cp_remaining", 0.0) or 0.0)
            if cp_rem <= 0.0:
                cp_rem = float(getattr(task, "duration", 0.0) or 0.0)

            oct_rem = max(cp_rem - float(getattr(task, "duration", 0.0) or 0.0), 0.0)
            ...
            for h in valid_hosts:
                ...
                eft = float(env.cur_time) + delay + float(getattr(task, "duration", 0.0) or 0.0)
                oeft = eft + oct_rem

                if task_best_oeft is None or oeft < task_best_oeft or (oeft == task_best_oeft and delay < float(task_best_delay)):
                    task_best_host = hid
                    task_best_oeft = oeft
                    task_best_eft = eft
                    task_best_delay = delay

            if task_best_host is None or task_best_oeft is None:
                continue

            if (
                best_oeft is None
                or task_best_oeft < best_oeft
                or (task_best_oeft == best_oeft and float(cp_rem) > float(best_cp))
                or (task_best_oeft == best_oeft and float(cp_rem) == float(best_cp) and str(getattr(task, "task_id", "")) < str(getattr(best_task, "task_id", "")))
            ):
                best_task = task
                best_host = task_best_host
                best_oeft = task_best_oeft
                best_eft = task_best_eft
    ...
    def select_action(
        ...
        return valid_host_ids[0], {"reason": "peft_select_action_fallback"}
